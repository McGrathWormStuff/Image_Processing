...
o.write('Folder,Name,Num,threshold,num_regions,est_len,')
for i in range(1, 11):
...
for abs_directory in [f.path for f in os.scandir(args.path) if f.is_dir()]:
    ...
    for filename in glob.glob('*.png'):
        ...
        sp = np.fft.fft(img_c1)
        freq0 = np.fft.fftfreq(img_c1.shape[0])
        freq1 = np.fft.fftfreq(img_c1.shape[1])
        print(freq0)
        print(freq1)
        ...
        # plt.savefig('fourier_transf_example.png')
