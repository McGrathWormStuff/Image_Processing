...
o.write('Folder,Name,Num,threshold,num_regions,mean_fourier_freq_real,mean_fourier_freq_imag,est_len,')
for i in range(1, 11):
...
for abs_directory in [f.path for f in os.scandir(args.path) if f.is_dir()]:
    ...
    for filename in glob.glob('*.png'):
        ...
        # Fourrier transformation frequencies
        # Fourier Transformation and plotting
        img_c1 = cv2.imread(filename, 0)
        img_c2 = np.fft.fft(img_c1)
        o.write(str(np.mean(img_c2).real) + ',' + str(np.mean(img_c2).imag) + ',')
