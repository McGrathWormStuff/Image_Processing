...
def FeatureSelection():
    ...
    plt.savefig('feature_selection/ldaa_weights_heatmap.png', bbox_inches = "tight")
    plt.show()
    # Output the weights to an excel file
    lda_weights.to_excel('feature_selection/ldaa_weights.xlsx')
    ...
    output['LDAA_Single_Loaded'] = lda1.predict(new_images_std)
    output['LDAA_Whole_Animal'] = lda2.predict(new_images_std)
    output['LDAA_Straight'] = lda3.predict(new_images_std)
    output['LDAA_Clear'] = lda4.predict(new_images_std)
    output['LDAA_Head_First'] = lda5.predict(new_images_std)
    output['SFSA_Single_Loaded'] = classifier1.predict(new_images_std[0:,list(sfs1.k_feature_idx_)])
    output['SFSA_Whole_Animal'] = classifier2.predict(new_images_std[0:,list(sfs2.k_feature_idx_)])
    output['SFSA_Straight'] = classifier3.predict(new_images_std[0:,list(sfs3.k_feature_idx_)])
    output['SFSA_Clear'] = classifier4.predict(new_images_std[0:,list(sfs4.k_feature_idx_)])
    output['SFSA_Head_First'] = classifier5.predict(new_images_std[0:,list(sfs5.k_feature_idx_)])
    # Output
