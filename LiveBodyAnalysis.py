...
# Ubuntu note for Faye: see all .png files in folder: find . -type f -regex ".*\.pdf"
...
for abs_directory in [f.path for f in os.scandir(args.path) if f.is_dir()]:
    ...
    for filename in glob.glob('*.png'):
        ...
        o.write(str(seg2int) + ',' + str(seg9int) + ',')

        # Tip width ratio of either end of the worm
        try:
            tip1 = ImSegGray[int(min(row)):int(max(row)),int(min(col)):int(min(col)+round(est_len/20))]
            tip2 = ImSegGray[int(min(row)):int(max(row)),int(min(col)+19*round(est_len/20)):int(min(col)+est_len)]
            tipWidth1 = SA.stats(tip1)[1]
